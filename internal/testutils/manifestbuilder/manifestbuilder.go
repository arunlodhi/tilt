...
func (b ManifestBuilder) WithLocalForceParallel(v bool) ManifestBuilder {
	b.localForceParallel = v
	return b
}
...
func (b ManifestBuilder) Build() model.Manifest {
	...
	if b.k8sYAML != "" {
		...
		lt := model.NewLocalTarget(
			...
			b.localDeps,
			b.f.Path()).
			WithForceParallel(b.localForceParallel)
